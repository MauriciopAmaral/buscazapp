...
export function FinanceiroClient() {
  ...
  const pedirRenovacao = searchParams.get("renovar") === "1";
  ...
  const [selecaoInicializada, setSelecaoInicializada] = useState(false);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      let periodicidadeAtual: Periodicidade = "mensal";
      if (jsonSub?.success) {
        planoAtual = jsonSub.data.planoId;
        periodicidadeAtual = jsonSub.data.subscription?.periodicidade ?? "mensal";
        setPlanoId(planoAtual);
      ...
      // Só define a seleção inicial do seletor "Trocar plano" uma vez (pra
      // não sobrescrever o que a pessoa já escolheu, se recarregar depois
      // de uma troca). Sincronizada com a periodicidade que já está ativa
      // — igual à tela Assinatura, pra não mostrar "Mensal" por padrão
      // quando a empresa já é trimestral/anual.
      if (!selecaoInicializada) {
        setNovaPeriodicidade(periodicidadeAtual);
        if (jsonPlanos?.success && jsonPlanos.data.length > 0) {
          // Vindo de um lembrete de renovação: mesmo plano, pra renovar o
          // período atual. Senão, sugere o primeiro plano diferente do
          // atual (pra não abrir o seletor já "travado" no próprio plano).
          const alvo = pedirRenovacao
            ? planoAtual
            : jsonPlanos.data.find((p: PlanoReal) => p.id !== planoAtual)?.id ?? jsonPlanos.data[0].id;
          setNovoPlanoId(alvo);
          if (pedirRenovacao) setTrocando(true);
        }
        setSelecaoInicializada(true);
      }
    } finally {
      setCarregando(false);
    }
  }, [token, selecaoInicializada, pedirRenovacao]);
      ...
      <div className="mt-6 rounded-2xl border border-ink-200 bg-white p-5">
        ...
        {trocando ? (
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <Button onClick={() => setTrocando(true)} disabled={!novoPlanoId}>
        ...
        {!trocando && novoPlanoId === planoId && (
          <p className="mt-2 text-xs text-ink-400">
            Esse é o seu plano atual — continuar renova o período (útil pra pagar antes do vencimento).
          </p>
        )}
      </div>
