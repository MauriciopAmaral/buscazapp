...
## Atualização: Painel da empresa → Impulsionar, agora pagando de verdade pelo Mercado Pago

Essa tela era só simulação — clicar em "Contratar" apenas mudava um estado local na tela, sem cobrar nada, sem salvar no banco e sem ativar destaque nenhum. Agora o fluxo completo é real:

- **Precisa rodar `npx prisma db push`** — entrou uma tabela nova, `Boost`, que guarda cada impulsionamento contratado (tipo, dias, valor, status do pagamento) e um novo enum `BoostStatus` (`pendente` / `pago` / `cancelado` / `expirado`). **Rode o `db push` antes de dar `git push`/fazer o deploy** — é a mesma lição de sempre: como o preço agora é calculado e validado no servidor a partir dessa tabela, se o banco de produção não tiver as colunas novas antes do deploy, a rota quebra.
- **Precisa de duas variáveis de ambiente novas na Vercel** (Project Settings → Environment Variables), do mesmo jeito que as `FTP_*` foram configuradas:
  - `MP_ACCESS_TOKEN` — o Access Token de **produção** da sua conta Mercado Pago (Developers → sua aplicação → Credenciais de produção). **Nunca coloque esse valor num arquivo do repositório** — ele fica só nas variáveis de ambiente da Vercel (e no seu `.env` local, que já é ignorado pelo git).
  - `SITE_BASE_URL` — a URL de produção do site, sem barra no final: `https://www.buscazap.com`. É usada pra montar as URLs de retorno do checkout e a URL que o Mercado Pago chama depois do pagamento.
  - A **Public Key** (`APP_USR-13fc...`) não é usada no backend — ela só seria necessária se um dia migrarmos pro Checkout **transparente** (formulário de cartão dentro do próprio site); como escolhemos o Checkout Pro (redireciona pro Mercado Pago), ela fica sem uso por enquanto.
- **Como funciona o fluxo, de ponta a ponta**:
  1. Na tela Impulsionar, a empresa escolhe o tipo de destaque (Destaque na home, Destaque na categoria, Destaque na cidade, Resultado patrocinado ou Promoção destacada) e a duração (7, 15 ou 30 dias). O preço é sempre recalculado no servidor a partir de uma tabela de preços fixa (`src/lib/boostCatalog.ts`) — o valor mostrado na tela é só informativo, o cliente nunca consegue pagar um valor diferente do real mesmo manipulando a requisição.
  2. Ao clicar em "Pagar com Mercado Pago", a API (`POST /api/painel/boosts`) cria um registro `Boost` com status `pendente` e pede ao Mercado Pago uma "preferência de pagamento" (Checkout Pro) — a empresa é redirecionada pra fora do site, pro checkout oficial do Mercado Pago, onde escolhe cartão ou Pix.
  3. Depois de pagar (ou desistir), o Mercado Pago traz a empresa de volta pra `/painel/impulsionar/retorno`, uma tela que fica consultando automaticamente (a cada 3s, por até ~30s) se o pagamento já foi confirmado, e mostra o resultado assim que souber — aprovado (com o impulsionamento já ativo), recusado, ou ainda aguardando (Pix pode levar poucos segundos; boleto, se algum dia for habilitado, até 3 dias úteis).
  4. Quem realmente confirma e libera o impulsionamento é um webhook (`POST /api/webhooks/mercadopago`) que o próprio Mercado Pago chama assim que o status do pagamento muda — não depende da pessoa ficar com a tela de retorno aberta. Por segurança, esse webhook nunca confia no que vem no corpo da notificação (qualquer um poderia forjar uma chamada pra essa URL); ele usa só o ID do pagamento avisado e busca de novo, direto na API do Mercado Pago com o Access Token, o status verdadeiro daquele pagamento. Só quando o Mercado Pago confirma "aprovado" é que o sistema cria o anúncio (`Ad`) de destaque pelo período contratado e marca o `Boost` como pago — de forma atômica (numa transação), pra nunca ficar um impulsionamento marcado como pago sem o destaque correspondente. Também é resistente a reenvio duplicado de notificação (o Mercado Pago reenvia às vezes) e a pagamentos recusados/cancelados (marca o `Boost` como cancelado, sem cobrar nem ativar nada).
  5. **Sobre o "aviso de pagamento" pro cliente**: hoje esse aviso é a própria tela de retorno com confirmação automática (item 3 acima) — a empresa vê na hora, sem precisar atualizar nada. Ainda **não** existe um aviso por e-mail/WhatsApp separado confirmando o pagamento; isso entra no mesmo grupo do que já está pendente pra notificações internas do admin e pro "esqueci minha senha" (precisa de um serviço de envio configurado, ex: Resend).
- Depois de configurar `MP_ACCESS_TOKEN` e `SITE_BASE_URL` na Vercel, ainda falta um passo manual **fora do código**, direto no painel do Mercado Pago: em Developers → sua aplicação → Webhooks, cadastre a URL `https://www.buscazap.com/api/webhooks/mercadopago` pra notificação de pagamentos (senão o pagamento fica "aprovado" no Mercado Pago mas o impulsionamento nunca é liberado sozinho — só quando a pessoa abre a tela de retorno e a consulta automática pega o status atualizado).

## Atualização: Impulsionar — pagamento embutido na página (sem redirecionar pro Mercado Pago) e só Pix + cartão

Depois de testar em produção, dois ajustes pedidos:

- **Não sair mais da página pra pagar.** A versão anterior usava o "Checkout Pro" do Mercado Pago, que redireciona a empresa pra fora do BuscaZapp durante o pagamento. Trocado pelo **Payment Brick** (checkout transparente/embutido): o formulário de pagamento aparece dentro da própria tela do Impulsionar, a empresa preenche os dados ali e nunca sai do site. Tecnicamente, isso trocou a "Preferência" (Checkout Pro) por criar o **Pagamento diretamente pela API** do Mercado Pago (`POST /v1/payments`), a partir dos dados que o formulário embutido tokeniza no navegador.
- **Só cartão de crédito e Pix** — o boleto (que o Checkout Pro oferecia sozinho, sem a gente escolher) foi desativado. O formulário embutido já vem configurado só com essas duas opções, e o servidor também recusa qualquer outro meio de pagamento por segurança, mesmo que alguém tente chamar a rota de pagamento manualmente.
- **Não muda o schema** — não precisa de `db push` nessa. **Precisa de mais uma variável de ambiente na Vercel**, além das duas anteriores:
  - `NEXT_PUBLIC_MP_PUBLIC_KEY` — a Public Key de produção da mesma aplicação Mercado Pago (a que começa com `APP_USR-13fc...`). Diferente do Access Token, essa é feita pra ir pro navegador — é o que carrega o formulário de pagamento embutido. Não tem problema nenhum ela aparecer no código do site publicado; é assim que o Mercado Pago espera que ela seja usada.
- **Como funciona agora, de ponta a ponta**:
  1. A empresa escolhe o tipo de destaque e a duração (agora com a opção de **1 dia** também, além de 7/15/30) e clica em "Continuar para pagamento". Isso só cria o registro `Boost` (`pendente`) com o preço calculado no servidor — ainda não cobra nada.
  2. Na mesma tela aparece o formulário do Mercado Pago (cartão de crédito ou Pix). Ao confirmar, os dados tokenizados vão pro backend (`POST /api/painel/boosts/[id]/pagar`), que cria o pagamento de verdade na API do Mercado Pago — o valor cobrado é sempre o do `Boost` já criado no servidor, nunca o que vier do navegador.
  3. **Cartão de crédito**: a resposta já vem na hora (aprovado ou recusado) — a tela mostra o resultado direto, sem sair da página.
  4. **Pix**: a resposta vem com o QR code (pra escanear) e o código "copia e cola". A tela mostra os dois e fica consultando sozinha a cada poucos segundos até o pagamento ser confirmado — quem confirma de fato é o webhook (`POST /api/webhooks/mercadopago`, sem mudança nessa parte), então funciona mesmo que a pessoa feche a aba antes de ver a confirmação (é só reabrir o histórico depois).
...
## Atualização: corrigido bug que podia deixar um impulsionamento pago "invisível" mesmo ativo

Depois do teste real de "Destaque na cidade" (Grafica Print Desing), a empresa não apareceu nem na home ("Empresas perto de você"), nem na busca filtrada por cidade, nem em "Empresas em destaque". Duas causas possíveis, uma delas era mesmo um bug de código:

- **Possível causa 1 (não é bug, é esperado)**: o impulsionamento testado foi o de **1 dia**, comprado em 09/09 e válido até 10/09 — dependendo da hora exata do teste, pode já ter vencido. Isso é o comportamento correto (o destaque só vale dentro do prazo pago); pra confirmar que está tudo certo, o jeito mais simples é comprar um novo impulsionamento (mesmo o de 1 dia de novo) e testar enquanto ele ainda está "ativo" no histórico da tela Impulsionar.
- **Possível causa 2 (bug de verdade, corrigido agora)**: nas funções que decidem quem aparece em "Empresas perto de você" (`getCompanies`) e nas ofertas em destaque (`getActivePromotions`), a forma de buscar os dados no banco tinha uma falha: quando existia um impulsionamento ativo, a busca pedia só "a quantidade normal de resultados + 1 a mais por impulsionamento" (`take: take + impulsionadas.size`), na esperança de que isso fosse suficiente pra incluir a empresa impulsionada. Mas isso só funciona se a empresa impulsionada já estivesse **perto do topo** da ordem normal (mais bem avaliada, cadastro mais recente); se ela estivesse mais pra trás na ordem normal (cadastro mais antigo, poucas avaliações — como pode ser o caso de uma empresa nova testando o recurso), o corte acontecia antes dela ser promovida pra frente, e ela nunca aparecia. A função `getFeaturedCompanies` (home → "Empresas em destaque") já tinha sido feita do jeito certo desde o início; agora `getCompanies` e `getActivePromotions` usam a mesma lógica correta: quando tem impulsionamento ativo, busca **todo mundo** que bate com o filtro (sem cortar por quantidade), coloca os impulsionados na frente, e só **depois** corta pra quantidade que a tela pede.
...
## Atualização: botão "Planos" na Home + compra de plano de verdade (com pagamento, e-mail de cadastro e e-mail de boas-vindas)

Pedido do cliente: um botão "Planos" na página principal, com opção de "Adquirir plano" pra empresa; depois do pagamento aprovado, o admin tem que ver os dados desse pagamento, e mandar um e-mail com um link pra pessoa completar o cadastro da empresa; depois do cadastro, mandar outro e-mail confirmando "cadastro realizado com sucesso" com login e senha, agradecendo pela adesão.

Antes disso não existia nenhum jeito de comprar um plano de verdade — a seção "Planos" da página "Para empresas" já existia, mas só mostrava preços fictícios e o botão ia direto pro cadastro grátis, sem cobrar nada. E **não existia envio de e-mail nenhum no projeto** (nem o "esqueci minha senha" manda e-mail — só mostra o link na tela). Agora:

- **Botão "Planos" no menu do site** (`src/components/layout/Header.tsx`), que leva direto até a seção de planos em "Para empresas" (`/para-empresas#planos`).
- **A seção de Planos agora mostra os preços reais**, vindos do banco (`GET /api/planos`, público) — a mesma tabela `Plan` que o Admin → Planos já gerencia. Antes vinha de dados fictícios (`src/mocks/subscriptions.ts`), que nem batiam com os preços reais cadastrados no admin.
- **"Adquirir plano" abre uma tela de pagamento embutida** (`/planos/comprar?plano=<id>`), sem precisar ter conta ainda: a pessoa escolhe a periodicidade (mensal/trimestral/anual), preenche nome e e-mail, e paga com cartão de crédito ou Pix, sem sair do site — mesmo Payment Brick do Impulsionar. O valor cobrado é sempre o do banco, nunca o que vier do navegador.
- **Tabela nova `PlanPurchase`** guarda cada compra (plano, periodicidade, valor, nome/e-mail/telefone de quem comprou, status do pagamento) — como ainda não existe conta nem empresa nesse momento, essa compra fica "solta" até o cadastro ser concluído.
- **Depois que o pagamento é aprovado**, um e-mail é mandado pro cliente com um link (`/cadastro?planoToken=...`) pra completar o cadastro — a tela de sucesso do checkout também mostra um botão "Completar cadastro agora" com o mesmo link, pra quem não quiser esperar o e-mail chegar.
- **Ao completar o cadastro** (`POST /api/auth/register`, com o token do link), a conta é criada normalmente e fica vinculada à compra do plano. Quando a pessoa termina de cadastrar a empresa (`POST /api/painel/company/create`, passo seguinte do fluxo), o plano contratado é aplicado automaticamente na empresa (cria a `Subscription` com o plano e o preço certos, em vez de nascer no plano Gratuito padrão).
- **E-mail "cadastro realizado com sucesso"**: mandado assim que a conta é criada (ainda dentro de `POST /api/auth/register`, só quando veio de uma compra de plano), com login (o e-mail) e a senha que a pessoa acabou de escolher no formulário, mais um agradecimento pela adesão. A senha só existe em texto puro nesse momento exato (antes de virar hash) — não fica salva em lugar nenhum, é só reenviada de volta uma vez por e-mail, como pedido.
- **Admin → Financeiro agora também mostra as compras de plano**, do mesmo jeito que já mostra o Impulsionar — inclusive as que ainda estão "aguardando cadastro" (pagas, mas a pessoa ainda não completou o cadastro). Os botões de marcar como pago/pendente e excluir também funcionam nessas linhas agora (antes só funcionavam pra pagamento de assinatura "clássico"; a mesma limitação existia silenciosamente pro Impulsionar desde a atualização anterior — aproveitei e corrigi os dois juntos).
- **Envio de e-mail de verdade**: novo `src/lib/emailSender.ts`, usando a API HTTP do Resend (sem precisar instalar SDK). **Precisa de duas variáveis de ambiente novas na Vercel**:
  - `RESEND_API_KEY` — sua chave de API do Resend (crie a conta em resend.com, é gratuito pra o volume de um projeto assim).
  - `RESEND_FROM_EMAIL` — o remetente, no formato `"BuscaZapp <contato@buscazap.com>"`, usando um domínio **verificado** no Resend (Domains → Add domain — sem isso o Resend recusa o envio).
  - Sem essas variáveis configuradas, o projeto continua funcionando normalmente (pagamento, cadastro, tudo) — só que o e-mail não é mandado de verdade, fica só registrado no log do servidor. Isso é intencional: nenhuma regra de negócio importante depende do e-mail ter sido enviado com sucesso.
...
## Atualização: corrigido "Continuar para pagamento" que parecia travado, sincronizado Financeiro com Assinatura, e lembrete de vencimento por e-mail

Três ajustes pedidos depois do teste da atualização anterior:

- **"Continuar para pagamento" (Financeiro) e "Assinar" (Assinatura) agora vão direto pro pagamento.** Causa: depois de clicar, a tela abria um card que só mostrava o plano escolhido e pedia OUTRO clique em "Continuar" — parecia travado, porque visualmente parecia a mesma tela. Agora esse segundo clique não existe mais: assim que o card de troca de plano aparece, ele já dispara a criação da cobrança sozinho e mostra o formulário de pagamento (ou, no caso do plano Gratuito, já aplica a troca na hora).
- **Financeiro sincronizado com Assinatura**: a periodicidade sugerida no seletor "Trocar plano" do Financeiro agora começa igual à periodicidade que já está ativa (antes sempre começava em "Mensal", mesmo se a empresa já fosse trimestral/anual). Também não sugere mais o próprio plano atual como primeira opção (o que deixava o botão sem fazer nada por padrão) — sugere o primeiro plano diferente. Escolher o mesmo plano atual continua possível e agora funciona como "renovar o período" (útil pra pagar antes do vencimento), em vez de ficar bloqueado.
- **Lembrete de vencimento por e-mail**: todo dia, uma tarefa agendada (`vercel.json` → Vercel Cron Jobs) verifica quais assinaturas vencem nos próximos 3 dias e manda um e-mail com o link "Pagar agora" — que leva direto pro Financeiro já com o pagamento do mesmo plano/período aberto (`/painel/financeiro?renovar=1`). Como o sistema não guarda o cartão pra cobrar sozinho (cada pagamento é feito pela própria empresa, no formulário embutido), esse lembrete por e-mail é o mecanismo real de cobrança recorrente — sem ele, a assinatura simplesmente ficaria "vencida" sem ninguém saber.
  - **Precisa de uma variável de ambiente nova, opcional mas recomendada**: `CRON_SECRET` — uma senha aleatória qualquer (gere com `openssl rand -hex 32`), configurada tanto como variável de ambiente normal quanto em Project Settings → Cron Jobs na Vercel. Sem ela, a rota do lembrete funciona sem checar nada (dá pra testar assim), mas fica aberta pra qualquer um na internet chamar e disparar e-mails pros seus clientes.
  - Usa o mesmo `RESEND_API_KEY`/`RESEND_FROM_EMAIL` já configurado pra e-mails de plano — não precisa configurar de novo.
...
## Atualização: auditoria de lançamento no site inteiro (fora do Admin) — corrigida uma falha de segurança séria

Depois de deixar o Admin pronto, fiz uma segunda auditoria — agora no resto do site (público + painel da empresa) — pra responder direto "falta algo pra lançar?". Encontrei e já corrigi os seguintes pontos:

**🔴 Falha de segurança séria, já corrigida — ação sua ainda necessária (leia o aviso abaixo):** a tela pública de cadastro (`/cadastro`) tinha um bloco "Atalhos de desenvolvimento" com três botões — "Entrar como consumidor", "Entrar como empresa" e **"Entrar como administrador"** — que logavam automaticamente em contas de teste fixas criadas pelo `prisma/seed.ts` (senha `123456` pra todas, incluindo a de admin, `admin@buscazapp.com.br`). Isso significa que **qualquer visitante do site, sem digitar nada, conseguia clicar num botão e entrar como administrador**. Removi o bloco inteiro da tela pública. **Mas atenção: se você já rodou `npm run db:seed` apontando pro banco de produção (Hostinger) em algum momento, essa conta de admin com a senha `123456` existe de verdade no seu banco agora — troque a senha dela (ou apague e crie uma nova conta de admin) antes de divulgar o site.** Da mesma forma, o seed também popula o banco com dezenas de empresas, produtos, avaliações etc. fictícios (os mesmos que estavam em `src/mocks`) — se voce rodou o seed em produção, vale conferir se essas empresas de mentira precisam ser removidas antes do público ver a busca.

**Reivindicar empresa estava completamente quebrado para empresas de verdade**: as duas telas de `/reivindicar` (a busca e a página de cada empresa) ainda liam de `src/mocks/companies` em vez do banco — então o botão "Reivindicar perfil" que aparece na página de qualquer empresa real levava pra um "não encontrado". Corrigido: as duas telas agora buscam a empresa de verdade (`GET /api/companies` pra busca, e a mesma função que já abastece a página pública da empresa pra tela de reivindicação em si).

**Textos públicos que diziam "protótipo com dados fictícios"**: removidos/reescritos — estavam no rodapé de toda página, na descrição do site (aparece no Google e ao compartilhar o link), na página do Clube, do Cashback, e na "Para empresas" (que também tinha depoimentos inventados de empresas fictícias — removi essa seção inteira, em vez de só trocar o aviso, porque depoimento inventado apresentado como real não é uma boa prática mesmo com aviso). A página "Para empresas" também mostrava números fixos inventados (quantidade de empresas, cidades, cliques no WhatsApp/mês, avaliação média, e o "simulador de alcance") — agora usa números reais vindos do banco (nova rota pública `GET /api/public-stats`).

**Selo "Promoção"/"Cupom" nos cards de empresa nunca aparecia pra empresa real**: os cards (home, busca, categoria, Clube) checavam um array de mock em vez do banco, então mesmo uma empresa com promoção/cupom ativos de verdade nunca mostrava o selo. Corrigido: os selos agora vêm de dado real (calculado direto no servidor).

**Botão "Quero assinar o Clube"** também fazia login automático numa conta de demonstração (mesmo problema dos atalhos de dev, só que sem escolha de senha). Como uma assinatura paga recorrente pra consumidor final é uma funcionalidade nova e maior (fora do que já existe pra empresas), não implementei o cobrança agora — troquei o botão por um aviso honesto "Assinatura do Clube em breve", sem fingir que faz algo que não faz.

Não precisa de `npx prisma db push` por causa desta atualização — só mudei o **valor padrão** do texto do rodapé no schema (`rodapeTexto`), o que só afeta configurações novas; se seu banco já tem uma linha de configuração salva, o texto antigo continua lá até você editá-lo em Admin → Configurações (ou eu troco pra você se preferir).
